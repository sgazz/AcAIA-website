...
const generateExam = async (req, res) => {
    try {
        ...
        const aiExam = await aiService_1.aiService.generateExamSimulation({
            subject,
            difficulty,
            numberOfQuestions,
            duration
...
const getExams = async (req, res) => {
    try {
        ...
        const exams = await Exam_1.Exam.find(query)
            .sort({ createdAt: -1 })
            .limit(Number(limit))
...
const getExamStats = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user._id;
        const exam = await Exam_1.Exam.findOne({ _id: id, isActive: true, createdBy: userId });
        if (!exam) {
            res.status(404).json({
                success: false,
                message: 'Ispit nije pronađen.'
